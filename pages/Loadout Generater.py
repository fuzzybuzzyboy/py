...
with Loadout:
    ...
    if st.button('Save Config'):
        if os.path.exists(f"{os.path.join('configs', 'Loadout_generator_config')}.txt"): os.remove(f"{os.path.join('configs', 'Loadout_generator_config')}.txt")
        f = open(f"{os.path.join('configs', 'Loadout_generator_config')}.txt", "a")
        ...
        st.success(f'Saved config to file \'{os.path.join('configs', 'Loadout_generator_config')}.txt\'', icon="✅")
        f.close()
    ...
    if st.button('Load Config'):
        if os.path.exists(f"{os.path.join('configs', 'Loadout_generator_config')}.txt"): 
            ...
            if len(lines) == 33: 
                ...
                st.success(f"Loaded configuration from \'{os.path.join('configs', 'Loadout_generator_config')}.txt\'", icon="✅")
